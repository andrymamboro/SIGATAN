...
const MapPicker = React.lazy(() => import('@/components/maps/MapPicker'));
import { numberToWordsIDR, numberToWordsArea } from '@/components/utils/NumberToWords';
import { getPejabatByWilayah } from '@/api/pejabat';
import { getAgeFromDate, formatUmur, getUmurSampaiTahun } from '../../lib/utilsUmur';
...
export default function TanahForm({ tanah, onSubmit, isLoading, wilayahList = [], allTanahData = [], user }) {
      ...
      // Field koordinat polygon/polyline (readonly, selalu tampil di atas form, debug tempMapCoords)
      
    // State untuk koordinat sementara dari MapPicker
    const [tempMapCoords, setTempMapCoords] = useState(null);
    // Trigger rerender setelah update koordinat
    const [coordUpdate, setCoordUpdate] = useState(0);
    // State untuk popup umur
    const [showUmurPopup, setShowUmurPopup] = useState(null);
  ...
  // Tambahkan state untuk backup koordinat sebelum edit
  const [backupCoords, setBackupCoords] = useState(null);
  ...
  useEffect(() => {
    if (tanah) {
      ...
      if (!Array.isArray(loadedData.polygon_coords)) loadedData.polygon_coords = null;
  ...
  // Saat buka MapPicker, backup koordinat lama
  const openMapPicker = () => {
    setBackupCoords({
  ...
  return (
    <>
      {/* Selalu tampilkan hasil koordinat polygon/polyline di atas form */}
            ...
            <Card className="border-0 shadow-md">
              ...
              <CardContent className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button type="button" size="icon" variant="outline" className="h-9 w-9 p-0" onClick={() => setShowUmurPopup({ field: 'pemilik', value: formData.tempat_tgl_lahir_pemilik })}>
                    <span role="img" aria-label="Hitung Umur">🧮</span>
                  </Button>
                </div>
            ...
            <Card className="border-0 shadow-md">
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-center mt-2">
                  <span className="text-sm text-slate-500 "><span className="font-medium">Step : 2 of 4 </span></span>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
              ...
              <CardContent className="p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button type="button" size="icon" variant="outline" className="h-9 w-9 p-0" onClick={() => setShowUmurPopup({ field: 'penerima', value: formData.tempat_tgl_lahir_penerima })}>
                      <span role="img" aria-label="Hitung Umur">🧮</span>
                    </Button>
                  </div>

                        {/* Popup Umur (selalu di-root, agar tidak ter-clip parent) */}
                        {showUmurPopup && (
                          <Dialog open={!!showUmurPopup} onOpenChange={() => setShowUmurPopup(null)}>
                            <DialogContent className="max-w-md bg-white">
                              <DialogHeader>
                                <DialogTitle>Hitung Umur</DialogTitle>
                              </DialogHeader>
                              <div className="space-y-2">
                                <div className="text-sm text-slate-700">Masukkan tempat dan tanggal lahir (format: Palu, 20 Desember 1980 atau 20-12-1980):</div>
                                <Input
                                  value={showUmurPopup.value || ''}
                                  onChange={e => setShowUmurPopup(p => ({ ...p, value: e.target.value }))}
                                  placeholder="Palu, 20 Desember 1980"
                                />
                                <div className="text-blue-700 font-bold mt-2">
                                  {(() => {
                                    try {
                                      const umurStr = formatUmur(showUmurPopup.value);
                                      return umurStr && umurStr !== '-' ? `Umur: ${umurStr}` : 'Format tanggal tidak dikenali';
                                    } catch {
                                ...
                                <div className="flex gap-2 justify-end mt-4">
                                  <Button variant="outline" onClick={() => setShowUmurPopup(null)}>Tutup</Button>
                                  <Button
                                    onClick={() => {
                                      if (showUmurPopup.field === 'pemilik') handleChange('tempat_tgl_lahir_pemilik', showUmurPopup.value);
                                      if (showUmurPopup.field === 'penerima') handleChange('tempat_tgl_lahir_penerima', showUmurPopup.value);
                                      setShowUmurPopup(null);
                                    }}
                                    variant="default"
                                  >Pakai</Button>
                                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 justify-end">
                        ...
                        onClick={() => {
                          if (backupCoords) {
                            setFormData(prev => ({
                              ...
                              polygon_coords: backupCoords.polygon_coords,
                              location_type: backupCoords.location_type
                            }));
                          }
                        }}
                      >
                      ...
                      {backupCoords && (
                        ...
                        >
                          Kembalikan Koordinat Sebelumnya
                        </Button>
                      )}
                    </div>
                    ...
                    {(formData.location_type === 'polygon' || formData.location_type === 'polyline') && (
                      <div className="mt-4">
                        ...
                        {Array.isArray(formData.polygon_coords) && formData.polygon_coords.length > 0 ? (
                          <div className="max-h-48 overflow-y-auto flex flex-col gap-1">
                            {formData.polygon_coords.filter(c => Array.isArray(c) && c.length === 2).map((coord, idx) => (
                    ...
                    {/* Show centroid for polygon/polyline as pusat */}
                    {(formData.latitude && formData.longitude) && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        <p className="text-sm text-blue-800">
                          ...
                          {formData.location_type === 'polygon' || formData.location_type === 'polyline'
                            ? (() => {
                              // Calculate centroid for polygon/polyline
                              const coords = formData.polygon_coords;
                              if (Array.isArray(coords) && coords.length > 0) {
                                let centroid = { lat: 0, lng: 0 };
                                if (coords.length === 1) {
                                  centroid = { lat: coords[0][0], lng: coords[0][1] };
                                } else {
                                  // Calculate centroid for polygon/polyline
                                  ...
                                  centroid.lat = sumLat / coords.length;
                                  centroid.lng = sumLng / coords.length;
                                }
                                return `Polygon/Polyline (Pusat): ${centroid.lat}, ${centroid.lng}`;
                              }
                              return `Polygon/Polyline (Pusat): ${formData.latitude}, ${formData.longitude}`;
                            })()
                        ...
                        {(formData.location_type === 'polygon' || formData.location_type === 'polyline') && formData.polygon_coords && (
                          <p className="text-xs text-blue-700 mt-1">
                            ✓ {formData.location_type.charAt(0).toUpperCase() + formData.location_type.slice(1)} dengan {formData.polygon_coords.length} titik koordinat
                          </p>
                        )}
                      </div>
                    )}
      ...
      {/* Popup Umur (selalu di-root, agar tidak ter-clip parent dan tidak tergantung tab) */}
      {showUmurPopup && (
        <Dialog open={!!showUmurPopup} onOpenChange={() => setShowUmurPopup(null)}>
          <DialogContent className="max-w-md bg-white">
            <DialogHeader>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 mt-2">
                <Label className="text-blue-700 font-bold">Tahun Penguasaan Langsung</Label>
                <Input
                  type="number"
                  min="1900"
                  max={new Date().getFullYear() + 10}
                  value={showUmurPopup.tahunPenguasaan || ''}
                  onChange={e => setShowUmurPopup(p => ({ ...p, tahunPenguasaan: e.target.value }))}
              ...
              <div className="text-blue-700 font-bold mt-2">
                {(() => {
                  try {
                    const umurStr = formatUmur(showUmurPopup.value);
                    return umurStr && umurStr !== '-' ? `Umur sekarang: ${umurStr}`   : 'Format tanggal tidak dikenali';
                  } catch {
                    return 'Format tanggal tidak dikenali';
                  }
                })()}
              </div>
              {showUmurPopup.tahunPenguasaan && showUmurPopup.value && (
                <div className="text-green-700 font-bold mt-2">
                  {(() => {
                    const umurTahun = getUmurSampaiTahun(showUmurPopup.value, Number(showUmurPopup.tahunPenguasaan));
                    return (umurTahun !== null && !isNaN(umurTahun))
                      ? `Umur pada tahun ${showUmurPopup.tahunPenguasaan}: ${umurTahun} tahun`
                      : 'Format tanggal/tahun tidak valid';
                  })()}
                </div>
              )}
              <div className="flex gap-2 justify-end mt-4">
      ...
      {/* Map Picker Modal */}
      {showMapPicker && (
        <React.Suspense fallback={<div>Memuat peta pemilihan lokasi...</div>}>
          <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-2 md:p-6" onClick={() => setShowMapPicker(false)}>
            <div className="bg-gradient-to-br from-purple-300 via-fuchsia-110 to-indigo-100 rounded-lg shadow-xl w-full max-w-[96vw] md:max-w-[1300px] max-h-[96vh] overflow-hidden border-4 border-blue-600" onClick={(e) => e.stopPropagation()}>
              <div className="overflow-y-auto" style={{ maxHeight: 'calc(96vh - 40px)' }}>
                <MapPicker
                  value={{
                    latitude: tempMapCoords?.latitude ?? formData.latitude ?? (tanah && tanah.latitude) ?? '',
                    longitude: tempMapCoords?.longitude ?? formData.longitude ?? (tanah && tanah.longitude) ?? '',
                    polygon_coords: tempMapCoords?.polygon_coords ?? formData.polygon_coords ?? (tanah && tanah.polygon_coords) ?? null
                  }}
                  mapCenter={formData.latitude && formData.longitude ? [formData.latitude, formData.longitude] : undefined}
                  mapZoom={formData.latitude && formData.longitude ? 18 : undefined}
                  onMapClick={(coords) => {
                    setTempMapCoords({
                      latitude: coords.latitude,
                      longitude: coords.longitude,
                      polygon_coords: coords.polygon_coords || null,
                      type: coords.type || (coords.polygon_coords ? 'polygon' : 'marker'),
                    });
                  }}
                />
                <div className="flex justify-end gap-2 mt-2">
                  <Button type="button" onClick={() => setShowMapPicker(false)} variant="outline">Tutup</Button>
                  <Button type="button" variant="default" className="bg-white-50 text-blue font-bold hover:bg-white hover:text-green-500" 
                    onClick={() => {
                      if (tempMapCoords) {
                        setFormData(prev => {
                          let coords = tempMapCoords.polygon_coords;
                          if (coords && typeof coords === 'string') {
                            try { coords = JSON.parse(coords); } catch { coords = null; }
                          }
                          // Marker
                          if (tempMapCoords.type === 'marker') {
                            setCoordUpdate(c => c + 1);
                            setLastFormDataUpdate({
                              tempMapCoords,
                              before: prev,
                              after: {
                                ...prev,
                          ...
                          // Polygon/Polyline
                          if ((tempMapCoords.type === 'polygon' || tempMapCoords.type === 'polyline') && Array.isArray(coords)) {
                            // Hitung centroid
                            let lat = tempMapCoords.latitude;
                            let lng = tempMapCoords.longitude;
                            if (coords.length > 0) {
                              let sumLat = 0, sumLng = 0;
                            ...
                            setLastFormDataUpdate({
                              ...
                              after: {
                                ...
                                location_type: tempMapCoords.type,
                              }
                            });
                            return {
                              ...prev,
                          ...
                          // Default fallback
                          setCoordUpdate(c => c + 1);
                          setLastFormDataUpdate({ tempMapCoords, before: prev, after: prev });
                          return prev;
                        });
                    ...
                    {totalPages > 1 && (
                      <div className="flex items-center justify-between pt-4 border-t">
                        ...
                        <span className="text-sm text-slate-600">
                          Hal {asalUsulPage} dari {totalPages}
                        </span>
                        <Button
                          variant="outline"
                          size="sm"
